...
import { ciptaAduan, type HasilMaintenance } from "./actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";

const awalan: HasilMaintenance = {};

export function AduanForm({ unit }: { unit: { id: string; label: string }[] }) {
  const [hasil, tindakan, menunggu] = useActionState(ciptaAduan, awalan);
    ...
    <form action={tindakan} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          {hasil?.medan?.unitId && <p className="text-xs text-destructive">{hasil.medan.unitId}</p>}
        </div>
