...
import { rekodBayaranPenyewa, type HasilBayaranPenyewa } from "../actions";
import { LABEL_KAEDAH } from "@/lib/labels";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Button } from "@/components/ui/button";

const awalan: HasilBayaranPenyewa = {};

export function BayaranForm({ invoiceId, baki }: { invoiceId: string; baki: number }) {
  const router = useRouter();
  const borang = useRef<HTMLFormElement>(null);
  const [hasil, tindakan, menunggu] = useActionState(rekodBayaranPenyewa, awalan);

  useEffect(() => {
    if (hasil?.berjaya && !menunggu) {
      toast.success("Pembayaran dihantar — menunggu pengesahan tuan rumah");
  ...
  return (
    <form ref={borang} action={tindakan} className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="bukti">Bukti Bayaran (maks 5MB)</Label>
      ...
      <Button type="submit" disabled={menunggu} className="w-full">
        {menunggu ? "Menghantar..." : "Hantar Bukti Bayaran"}
      </Button>
    </form>
  );
}
